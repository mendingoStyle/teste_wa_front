...
const validationSchema = yup.object().shape({
  ...
  quantidade: yup.string().required().min(3).max(50),
  valor: yup.string().required().max(50),
  ...
  const formik = useFormikObservable<IOrder>({
    ...
    onSubmit(model) {
      return orderService.save(model).pipe(
        tap(order => {
          Toast.show(`${order.descricao} foi salvo'}`);
  ...
  return (
    ...
    >
      ...
      <form onSubmit={formik.handleSubmit}>
        <DialogTitle>{formik.values.id ? 'Editar' : 'Novo'} Pedidos</DialogTitle>
          ...
          <Fragment>
            <Grid container spacing={2}>
              <Grid item xs={12} sm={6}>
                <TextField label='Descricao' name='descricao' formik={formik} />
              </Grid>
              <Grid item xs={12} sm={6}>
                <TextField label='Quantidade' name='quantidade' formik={formik} />
              </Grid>
            </Grid>
